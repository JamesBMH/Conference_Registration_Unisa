...
public:
    ...
    // Uses the metaobject system to get the registration type and adds it at the start of the QList<QString>
    // Converts the data in m_AttendeeList into a qlist<qstring> using their toString functions to pass the data to the registrationlistwriter
    QList<QString> dataParser();
    // Destroys the list
    ~RegistrationList();
private:
